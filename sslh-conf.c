...
int sslhcfg_cl_parse(int argc, char* argv[], struct sslhcfg_item* cfg) {
 	...
 	sslhcfg_rsync = arg_strn(NULL, "rsync", "<host:port>", 0, 10, "Set up rsync target"),
...
#define MAX_MATCH 10
    for (cl_i = 0; cl_i < sslhcfg_rsync->count; cl_i++) {
        regex_t preg;
        regmatch_t pmatch[MAX_MATCH];
        int res = regcomp(&preg, "(\\w+):(\\w+)", REG_EXTENDED);
        if (res) {
        ...
        res = regexec(&preg, sslhcfg_rsync->sval [cl_i], MAX_MATCH, &pmatch[0], 0);
        if (res) {
            fprintf(stderr, "--rsync %s: Illegal argument\n", sslhcfg_rsync->sval [cl_i]);
            exit(1);
        }
        ...
        for (i = 0; i < cfg->protocols_len; i++) {
            if (!strncmp(cfg->protocols [i].name, "rsync",
                         5)) {
                found = i;
                break;
            }
        ...
        param_len = 5;
        group->name = calloc(1, param_len + 1);
        memcpy(group->name, "rsync", param_len);
        param_len = pmatch[1].rm_eo - pmatch[1].rm_so;
        group->host = calloc(1, param_len + 1);
        memcpy(group->host, sslhcfg_rsync->sval [cl_i]+pmatch[1].rm_so, param_len);
        param_len = pmatch[2].rm_eo - pmatch[2].rm_so;
        group->port = calloc(1, param_len + 1);
        memcpy(group->port, sslhcfg_rsync->sval [cl_i]+pmatch[2].rm_so, param_len);
    }
